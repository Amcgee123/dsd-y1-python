...
def check_age(num_junior):
    ...
    # Make sure num_junior is an integer
    num_junior = int(num_junior)

    for i in range(num_junior):
        flag = True

        while flag:
            age = input('Please enter the age for junior ticket {} : '.format(i+1))  # i+1 looks nicer

            try:
                age = int(age)  # convert to integer once
            except ValueError:
                ...
                if age < 16:
                    ...
                    print("Sorry, age over 16. Ticket will be changed to adult.")
                    not_junior += 1  # increment the counter
                    flag = False
